...
def grade_documents(state, openai_api_key) -> Literal["generate", "rewrite"]:
    ...
    st.write(f"start grade state:", state) 
 ...
 Your task:
        ...
        Now, based on the above rules, grade the document as 'yes' or 'no'.
        """,
...
def define_agent_workflow(openai_api_key):
    ...
    # Define the nodes
    #workflow.add_node("agent", lambda state: agent(state, openai_api_key))
    #retrieve = ToolNode([retriever_tool])  # 검색을 실행하는 retrieve 노드
    #retrieve = lambda state: retrieve_and_add_to_state(state,openai_api_key)  # 수정된 retrieve 노드
    ...
    #workflow.add_node("combine", lambda state: combine_chunks(state))  # 추가된 combine 노드
    ...
    workflow.add_conditional_edges(
        ...
        {
            "generate": "generate",  # 유효성을 통과하면 combine으로 이동
            "rewrite": "rewrite",  # 유효성 평가 실패 시 rewrite로 이동
        },
    )
    st.write(f"Final state after retrieve_and_add_to_state: {state}")
    #workflow.add_edge("combine", "generate")  # combine 후 generate로 이동
    workflow.add_edge("generate", END)  # generate에서 종료
    workflow.add_edge("rewrite", "retrieve")  # rewrite 후 agent로 돌아가서 다시 검색 수행
...
def retrieve_and_add_to_state(state,openai_api_key):
    ...
    attempt = 0
    used_chunk_ids = state.get("used_chunk_ids", set())
    final_results = state.get("final_results", [])
    st.write(f"start retrieve_and_add_to_state state:", state) 
    while attempt < max_retries:
        ...
        # 검색 수행
        search_results = retriever.get_relevant_documents(query, k=4)
        st.write(f"Number of search results retrieved: {len(search_results)}")
        ...
        for result in search_results:
            try:
                ...
                if chunk_id and chunk_id not in used_chunk_ids:
                    ...
                    used_chunk_ids.add(chunk_id)  # 새로 추가된 청크 ID를 사용된 목록에 추가
            except json.JSONDecodeError:
                st.write("Error decoding JSON chunk content.")
        final_results.extend(new_results)
            # 중복 청크 추가 후 상태 업데이트
        state["used_chunk_ids"] = list(used_chunk_ids)  # 상태에 리스트 형태로 저장 (참조 유지)
    ...
    st.write(f"After retrieve state:", state) 
    return state
...
def rewrite(state, openai_api_key):
    ...
    st.write(f"before rewrite state:", state) 
    ...
    rewrite_prompt = PromptTemplate(
        ...
        Objective:
        1. Rewrite the question to improve clarity and modify its phrasing or structure.
        2. Use synonyms or rephrase key parts of the question.
        3. The rewritten question should stay true to the core idea but sound sufficiently different to potentially trigger different retrieval responses.

        Output Rules:
        - Maintain the essence of the question but rewrite it in a way that increases the potential for finding different information.
        - Include important keywords, but change their context or order to broaden the search scope.
    ...
    st.write(f"After rewrite state:", state) 
    # 상태 업데이트 후 반환
    return {"messages": state["messages"]}
...
def run_agent_workflow(query, openai_api_key):
    ...
    state = {
        "messages": [
            ...
            AIMessage(content="User has asked a question, initiating retrieval process.")  # 초기 AI 메시지 추가
        ],
        "used_chunk_ids": set(),  # 사용된 청크 ID를 추적
        "final_results": []       # 최종 검색 결과를 저장
    }
    ...
    # node_output은 현재 상태의 일부이므로, 전체 상태에 병합
        if isinstance(node_output, dict):
            # 'retrieve'라는 중첩된 키가 있으면 내부 값으로 교체
            if "retrieve" in node_output:
                node_output = node_output["retrieve"]

            for key, value in node_output.items():
                if key == "messages" and isinstance(value, list):
                    # messages 리스트의 중복 제거 로직 추가
                    existing_ids = {msg.id for msg in state.get("messages", [])}
                    new_messages = [
                    ...
                    elif isinstance(state[key], set) and isinstance(value, set):
            ...
            st.write("Unexpected node_output format:", node_output)

        # 병합 이후 현재 전체 상태 출력
        st.write("Complete output after streaming a node:", state)
